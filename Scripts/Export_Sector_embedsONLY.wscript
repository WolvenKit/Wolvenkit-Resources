// Exports Just the embedded files from StreamingSector files and all referenced files (recursively)
...
let exportEmbeddedExtensions = [".mesh",".w2mesh", ".xbm", ".mlmask", ".mlsetup",".cfoliage", ".streamingsector",".cminimap"];

if (!withmats){
	Logger.Info('Withmats false')
    jsonExtensions = [".app", ".ent", ".streamingsector",".cfoliage",".cminimap"];
    exportExtensions = [".mesh",".w2mesh",".physicalscene"];
    exportEmbeddedExtensions = [".mesh",".w2mesh",".cfoliage", ".streamingsector",".cminimap"];
}
